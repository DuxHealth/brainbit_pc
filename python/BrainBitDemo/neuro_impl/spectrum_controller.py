...
class SpectrumController:
    ...
    def process_data(self, brain_bit_data):
        ...
        for i in range(len(brain_bit_data)):
            o1Values.append(brain_bit_data[i].O1)
            o2Values.append(brain_bit_data[i].O2)
            t3Values.append(brain_bit_data[i].T3)
            t4Values.append(brain_bit_data[i].T4)
        self.maths['O1'].push_data(o1Values)
        self.maths['O2'].push_data(o2Values)
        self.maths['T3'].push_data(t3Values)
        self.maths['T4'].push_data(t4Values)
        for i in range(4):
            self.maths[BB_channels[i]].process_data()
