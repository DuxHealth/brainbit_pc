...
export default function SignalScreen({ navigation }: NativeStackScreenProps<any>) {
  ...
  function startSignal() {
    ...
    NEEGControllerInstance.signalReceivedCallback = (data) => {
      for (let i = 0; i < channelsCount; i++) {
        data.forEach((sample) => {
          if(samples.has(channelInfos[i])){
            if(samples.get(channelInfos[i])?.length >= window) {
              samples.get(channelInfos[i])?.pop();
            }
            samples.get(channelInfos[i])?.push(sample.Samples[i] * 1e3)
          } else {
            console.log("set", channelInfos[i])
            samples.set(channelInfos[i], [sample.Samples[i] * 1e3])
          }
        })
      }
    }
    NEEGControllerInstance.startSignal();
  }

  function stopSignal() {
    setIsSignal(!isSignal)
    NEEGControllerInstance.signalReceivedCallback = undefined
    NEEGControllerInstance.stopSignal()
  }
  ...
  return (
    <View style={{ marginTop: 10, flex: 1 }}>
      <Button title={isSignal ? 'Stop' : 'Start'}
        ...
        />
        <Text style={[styles.blackText]}>{currentShowChannel}</Text>
        <LineChart
          style={{ flex: 1 }}
          gridMin={-1e3}
          gridMax={1e3}
